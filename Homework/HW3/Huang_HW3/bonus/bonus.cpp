...
int main() {
    ...
    for (i = 0; i < N; i++) {
        for (k = 0; k < 3; k++) {
            v[i][k] = v[i][k] / sqrt(temp) * sqrt(128.7); // Initial temperature is set as 129K
        }
    }
    ...
    for (t = 0; t < STEP; t++) {
        ...
        for (i = 0; i < N; i++) {
            for (k = 0; k < 3; k++) {
                tempv[i][k] = v[i][k] + (f[i][k] / m[i] - ETA * v[i][k]) * dlt / 2; // Verlet loop, calculate the velocity of (t + dlt / 2)
            }
        }
        for (i = 0; i < N; i++) {
            for (k = 0; k < 3; k++) {
        ...
        for (i = 0; i < N; i++) {
            for (k = 0; k < 3; k++) {
                v[i][k] = (tempv[i][k] + f[i][k] * dlt / (2 * m[i])) / (1 + ETA * dlt / 2); // verlet loop, calculate the new velocity of (t + dlt)
            }
        }
        ...
        VMD_out << "256 \n";
        VMD_out << "256 Ar LJ Simulization\n";
        for (i = 0; i < N; i++)
        {
            VMD_out << "Ar " << r[i][0] << " "
                    << r[i][1] << " " << r[i][2] << "\n";
