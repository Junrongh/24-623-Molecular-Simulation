...
int main() {
    ...
    URC = getURC(RC);
    FRC = getFRC(RC);
    standardv = sqrt((T_INITIAL * KB * 3 * (N - 1)) / (EPSILON * N));
    cout << standardv << endl;
    ...
    for (t = 0; t < STEP; t++) {
        ...
        for (i = 0; i < N; i++) {
            temp = random(0, 1);
            if (temp <= dlt * nu) {
                tempa1 = random(0, 180) * PI / 180;
                tempa2 = random(0, 360) * PI / 180;
                v[i][0] = gaussrand(standardv * cos(tempa1), sqrt(KB * T_INITIAL / EPSILON));
                v[i][1] = gaussrand(standardv * sin(tempa1) * cos(tempa2), sqrt(KB * T_INITIAL / EPSILON));
                v[i][2] = gaussrand(standardv * sin(tempa1) * sin(tempa2), sqrt(KB * T_INITIAL / EPSILON));
            }
        }
    }
}
