...
const int N = 256, STEP = 200000;
const double dlt = 0.002, nu = 0.05;
const double L = 6.8, RC = 2.5, PI = 3.1415926;
...
int main() {
    ...
    double K, U, E; // Define energy and momenta
    double T, P;
    double vtotal[3]; // Support value for making system momenta to be 0
    double URC, FRC;
    double tempr, tempf, temp, tempvv, tempa1, tempa2;
    ...
    URC = getURC(RC);
    FRC = getFRC(RC);
    standardv = sqrt((T_INITIAL * 3 * KB) / (2 * EPSILON));
    cout << standardv << endl;
    ...
    for (i = 0; i < N; i++) {
        for (k = 0; k < 3; k++) {
            v[i][k] = v[i][k] / sqrt(temp) * sqrt(50.0);
        }
    }
    ...
    for (t = 0; t < STEP; t++) {
        ...
        K = kinetic(v, m);
        U = potential(r, URC, FRC);
        T = temperature(K) * EPSILON / KB;
        P = pressure(T, r, URC, FRC);
        total_T = total_T + T;
        total_P = total_P + P;
        momenta(M, v, m);
        energy_out << t*dlt << ' ' << U << ' ' << K << ' ' << U + K << endl;
        tp_out << t*dlt << ' ' << T << ' ' << total_T / (t + 1) << ' ' << P << ' ' << total_P / (t + 1) << endl;
        momenta_out << t*dlt << ' ' << M[0] << ' ' << M[1] << ' ' << M[2] << endl;
        kreg(r, v, m);
        for (i = 0; i < N; i++) {
            temp = random(0, 1);
            if (temp <= dlt * nu) {                
                tempa1 = random(0, 180) * PI / 180.0;
                tempa2 = random(0, 360) * PI / 180.0;
                v[i][0] = gaussrand(standardv * cos(tempa1), sqrt(KB * T_INITIAL / (2 * EPSILON)));
                v[i][1] = gaussrand(standardv * sin(tempa1) * cos(tempa2), sqrt(KB * T_INITIAL / (2 * EPSILON)));
                v[i][2] = gaussrand(standardv * sin(tempa1) * sin(tempa2), sqrt(KB * T_INITIAL / (2 * EPSILON)));
            }
        }
    }
}
